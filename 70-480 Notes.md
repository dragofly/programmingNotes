...
- Browser caching events, `window.applicationCache`
    - `:onchecking`
...
- Creating a web worker
    - `addEventListener`('message', f(e), true) 
        - listener for worker
    - `postMessage`(results) 
        - calls the .onmessage() int the UI thread
    ```javascript
...
- Closing worker
    ...
    - `.close()`
        - Waits for the worker to finish processing <br/>
...
- Server implementation
    ...
    - Implementation is done using Flek a C# library for handling WebSockets
    - C# implementation:
        - `.OnOpen()` 
            - add to sockets list
        - `.OnClose()` 
            - remove form socket list
        - `.OnMessage()` 
            - handle message form the client
        - `.Send()` 
...
- Standard HTML markup + vocabulary namespace used to anotate pages so that programs can interact with the page.
...
* exceptions
    ...
    - `try catch`: catches the error if it was thrown inside the try-catch block
...
* built in libraries
    ...
    - number
        ...
        - has an offset when working with decimals
...
- ajax
    ...
    - `$.ajax(configObject)`
        ...
        - `accepts`
            - what data types are valid response types (`Accepts` request header)
            - JS object
        - `dataType`
            - used by jquery to deserialize the object that is returned
        - `contentType`
